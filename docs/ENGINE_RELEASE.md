...
- CLI tools: `basert-serve`, `basert-chat`, `basert-complete`, `basert-bench`,
  `basert-inspect`, `basert-transcribe`.
- `include/baseRT/` — the public headers matching this release.

## How releases are produced

A workflow in the engine repo (`release.yml`) builds the engine on an
